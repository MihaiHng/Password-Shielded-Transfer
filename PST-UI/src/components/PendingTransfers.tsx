...
//         address: isERC20Token && chainId ? tokenAddress : undefined, // Only pass tokenAddress if isERC20Token and chainId are valid
//         abi: ERC20_CONTRACT_ABI,
//         functionName: 'symbol',
//         chainId: chainId,
//         query: {
//             enabled: isERC20Token && !!chainId, // Enable only if isERC20Token and chainId are valid
//             staleTime: Infinity,
//         }
...
//     // Conditionally define transferDetailsQueries only if pstContractAddressForChain and chain.id are available
//     // AND there are actual transfer IDs to map over.
//     const transferDetailsQueries = allRelatedTransferIds.map((transferId) => {
//         // Explicitly pass undefined for address/chainId if not ready
//         const contractAddress = areContractDetailsReady ? pstContractAddressForChain : undefined;
//         const chainId = areContractDetailsReady ? chain.id : undefined; // Safely access chain.id
...
//             chainId: chainId,
//             query: {
//                 enabled: areContractDetailsReady, // Enabled only if contract address and chain ID are ready
//                 staleTime: 10000,
//             },
//         });
//     });
...
//     let filteredPendingTransfers = allRelatedTransferIds
//         .map((transferId, index) => {
//             const queryResult = transferDetailsQueries[index];
//             // Ensure queryResult is not undefined before accessing its properties
//             if (!queryResult) {
//                 return null;
//             }
...
//             ) : isLoadingAllIds || isLoadingAnyDetails ? (
